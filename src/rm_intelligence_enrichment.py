...
def parse_funding_outlook(page: str) -> dict:
    ...
    if "Funding Outlook/Target" not in t:
        return {}
    if not re.search(r"Total funding volume expected at USD\s*10bn equivalent in 2025", t, re.I):
        return {}
...
def parse_liquidity_commitment(page: str) -> dict:
    ...
    if "Commitment to Liquidity" not in t:
        return {}
    out = {}
    if re.search(r"Continue issuing an average of 3 USD benchmarks annually", t, re.I): out["usd_benchmarks_per_year"] = 3
    if re.search(r"more than 130 investors bought KDB’s USD public trades in 2024", t, re.I): out["usd_public_trade_investors_2024_min"] = 130
    if re.search(r"US\$1\.25bn .* per tranche", t, re.I): out["post_ssa_avg_tranche_usd_bn"] = 1.25
    return out
...
def validate(info: dict) -> list[dict]:
    ...
    if all(v is not None for v in shares): add("fx_currency_mix_sums_100", abs(sum(shares) - 100.0) < 0.11, f"Currency mix sums to {sum(shares):.1f}%")
    sectors = info.get("sector_exposure", {})
    vals = list(sectors.values())
    if vals: add("sector_mix_sums_100", abs(sum(vals) - 100.0) < 0.11, f"Sector mix sums to {sum(vals):.1f}%")
...
def enrich(snapshot: dict, pages: list[str]) -> dict:
    ...
    info = {
        ...
        "provenance": {"funding_structure": source(7, "Basic Framework of KDB’s Business Operations"), "sector_exposure": source(7, "Basic Framework of KDB’s Business Operations"), "foreign_currency_funding": source(17, "Foreign Currency Funding Channels", "2020-2024"), "recent_issuance": source(19, "Foreign Currency Funding Track Record", "2025-01-23"), "liquidity_commitment": source(20, "Commitment to Liquidity", "2024 / 2025-YTD"), "funding_outlook": source(21, "Funding Outlook/Target", "2025 target")},
    }
    info["sector_exposure"] = {k: v for k, v in info["sector_exposure"].items() if v is not None}
    info["quality_checks"] = validate(info)
    info["signals"] = []
    if outlook.get("2025_total_funding_target_usd_bn_equiv"): info["signals"].append({"type":"funding","priority":"high","fact":"KDB targets USD10bn equivalent total funding in 2025.","rm_angle":"Ask how much remains, currency/tenor mix, and whether prefunding is contemplated."})
    if fx.get("usd_share_pct", 0) > 60: info["signals"].append({"type":"fx_ccs","priority":"high","fact":f"USD represented {fx['usd_share_pct']:.1f}% of 2020-2024 foreign-currency bond issuance.","rm_angle":"Probe diversification economics and alternative-currency / CCS appetite."})
    if info["sector_exposure"].get("manufacturing_pct", 0) > 40: info["signals"].append({"type":"sector","priority":"medium","fact":f"Manufacturing accounts for {info['sector_exposure']['manufacturing_pct']:.1f}% of operating exposure.","rm_angle":"Use Korean manufacturing outbound investment as a route into ASEAN syndication, PF and transaction-banking conversations."})
...
def main():
    ...
    snapshot = enrich(snapshot, pdf_pages())
